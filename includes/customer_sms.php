...
function sendCustomerSms(PDO $pdo, int $tenant, int $clientId, string $message, $helper = null): array {
    ...
    $stmt = $pdo->prepare('SELECT c.*, p.price AS package_price FROM clients c LEFT JOIN packages p ON p.id=c.package_id AND p.tenant_id=c.tenant_id WHERE c.id=? AND c.tenant_id=?');
    ...
    $result = $helper->send($client['phone'], $helper->renderPlaceholders($message, $client), $clientId);

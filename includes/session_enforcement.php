...
function enforceCustomerSessions(PDO $pdo, callable $log, array $services = ['pppoe', 'hotspot']): void
{
    ...
    foreach ($routers as $r) {
        ...
        // Recover sessions independently of the customer's captive browser.
        try {
            require_once __DIR__ . '/hotspot_device.php';
            recoverPaidHotspotSessions($pdo, $sweepApi, $tid, (int)$r['id'], $log);
        } catch (Throwable $e) {
            $log('Paid device reconnect scan unavailable: ' . $e->getMessage());
        }

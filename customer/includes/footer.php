    ...
    <!-- Last stylesheet also covers legacy page-local styles. -->
    <link rel="stylesheet" href="css/portal.css?v=<?php echo filemtime(__DIR__.'/../css/portal.css'); ?>">
    <nav class="mobile-nav" aria-label="Customer navigation">
    <?php foreach (['dashboard.php'=>'Home', 'packages.php'=>'Packages', 'payment.php'=>'Payments', 'account.php'=>'Account'] as $path=>$label): ?>
        <a href="<?= $path ?>" <?= basename($_SERVER['PHP_SELF']) === $path ? 'aria-current="page"' : '' ?>><?= $label ?></a>
    <?php endforeach; ?>
    </nav>

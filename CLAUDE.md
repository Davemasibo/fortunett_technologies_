...
The method matters as much as the tenant: an STK push goes to whichever shortcode held the credentials and *can* be decided from configuration; a paybill payment goes wherever the customer typed and cannot. `isStkPaymentMethod()` draws that line, and the repair tool's catch-all rule is narrowed to STK methods only.

**A hand-entered payment is never platform money, and does not look different.** `api/payments/record_manual.php` maps the operator's "M-Pesa" choice to `payment_method = 'mpesa'` — byte for byte what `stk_push.php` writes. So an ISP who takes payment into their own paybill and types the receipt into the system produces a row indistinguishable, by method alone, from a platform-sent STK push. That is how 97 of one tenant's hand-entered receipts (KES 66,090 of their own money) were booked as ours to disburse. `manuallyRecordedSql()` is the one definition of the marker — the paired `mpesa_transactions` row carries `merchant_request_id 'MANUAL-…'` and `result_desc 'Manual:…'` — and every rule excludes it. Undo with `--undo-manual`, which targets that exact signature across all tenants rather than assuming anything about a tenant.

**Owning a paybill does not exempt an STK payment.** The first attempt at this narrowing also required `!paybill_own`, which is wrong: a `paybill_no_api` paybill has no API behind it and cannot send an STK push either, so a tenant holding one still had their pushes go out on the platform's shortcode. The test for an STK row is `stk_own` and nothing else; `paybill_own` only bears on `mpesa_paybill` rows, and there only as AMBIGUOUS.

**An inactive gateway is invisible everywhere else.** `is_active = 0` reads exactly like "no gateway" in every query in the app, which is how an operator can be certain a tenant has a paybill while the system shows none. `tools/collection_type_audit.php` is the one place inactive rows are printed.
...
**Recording a payment by hand used to leave the customer disconnected.** `record_manual.php` inserted two rows and stopped — no expiry extension, no provisioning — so an ISP collecting to their own paybill got no automation at all and reconnected every customer manually. It now runs `process_payment_success()` like any callback, and tags `collection_type = 'direct'` explicitly so the row can never again be mistaken for an untagged one that merely fell back to the same default.

**A `paybill_no_api` gateway cannot auto-activate anything, and used to say the opposite.** Registration needs Daraja credentials — there is no API on a manual paybill to register a callback with. But `tenantC2BStatus()` only ever looked at `mpesa_api` rows, so a tenant whose only gateway was a manual paybill fell through to the default and was told *"You are collecting through the FortuNett shared paybill"* — while `customer/payment.php` was showing their customers that very paybill to pay into. Money sent there is captured by nothing and the customer is never reconnected. There is now a `manual_paybill` mode whose banner says exactly that and offers Import Statement, which is the only reconciliation route available until the tenant adds API credentials for that shortcode.
...
**A fatal past the auth gate is invisible to `curl`.** `billing.php` SELECTs `platform_invoices.amount_paid` at line 112 with no try/catch. On a deployment where `sql/migrations/2026-07-26-platform-collections.sql` was never applied, that threw `SQLSTATE[42S22] Unknown column 'amount_paid'` and **every tenant's billing page returned a blank 500** — while `curl` reported a healthy `302 → login.php`, because `redirectIfNotLoggedIn()` exits long before the query. Checking a page from outside proves nothing about the logged-in path; reproduce with a faked session (`$_SESSION['user_id']`) on the CLI instead. `ensurePlatformBillingSchema()` now repairs that migration in place, called from all three entry points in `includes/platform_billing.php` — inside the functions, not at include time, because some callers require that file from inside a function body where the global `$pdo` is not in scope, and passing null to a PDO-typed parameter would turn a missing migration into a TypeError.
